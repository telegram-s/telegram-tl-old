...
fun String.uFirstLetter() = if (this[0].isUpperCase())
    this
else
    this[0].toString().toUpperCase() + this.substring(1)

fun String.lFirstLetter() = if (!this[0].isUpperCase())
    this
else
    this[0].toString().toLowerCase() + this.substring(1)

fun String.lCamelCase(): String
{
    var words = divideByWords(this)
    return words.iterator().drop(1).fold(words.get(0).lFirstLetter(), {(r, t) -> r + t.uFirstLetter() });
}
...
{
    var words = divideByWords(this)
    return words.iterator().fold("", {(r, t) -> r + t.uFirstLetter() });
}

private fun divideByWords(value: String): List<String>
{
    val dividers = array('.', '_')
