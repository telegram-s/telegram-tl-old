...
fun checkType(t: TLType)
{
    ...
    {
        if ((t as TLTypeGeneric).name != "Vector")
            throw RuntimeException("Only `Vector` generics are supported");
    }
}
...
fun getTypeReference(sourceType: TLType, types: HashMap<String, TLTypeDef>): TLTypeDef
{
    ...
    {
        var generic = sourceType as TLTypeGeneric
        TLBuiltInGenericTypeDef(generic.name, getTypeReference(generic.generics.get(0), types))
    }
